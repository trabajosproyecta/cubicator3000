...
class Gui(Frame):
    ...
    def initUI(self):
        ...
        label_title = Label(
            self, text="Cubicador Proyecta!", background="white",
            foreground="cyan", width=25, anchor=N, borderwidth=5,
            relief=GROOVE, font=("default", 16, "bold"))
        ...
        input_button = Button(
            self, text="Abrir",
            command=lambda: self.get_excel_file(self.entry_input))
        ...
        output_button = Button(self, text="Abrir",
                               command=lambda: self.get_dir(self.entry_output))
        ...
        images_button = Button(self, text="Abrir",
                               command=lambda: self.get_dir(self.entry_images))
        ...
        startButton = Button(self, text="Cubicar!",
                             command=lambda: self.run_optimize())
    ...
    def get_excel_file(self, entry):
        fname = askopenfilename(filetypes=(("Excel files", "*.xls"),
                                           ("Excel files", "*.xlsx")))
        if fname:
            entry.delete(0, END)
